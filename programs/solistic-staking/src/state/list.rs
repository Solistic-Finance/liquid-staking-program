...
use crate::{error::SolisticError, require_lt};
...
impl List {
    ...
    pub fn capacity(&self, account_len: usize) -> Result<u32> {
        ...
        .map_err(|_| error!(SolisticError::CalculationFailure))?
    ...
    pub fn get<I: AnchorDeserialize>(&self, data: &[u8], index: u32) -> Result<I> {
        require_lt!(index, self.len(), SolisticError::ListIndexOutOfBounds);
    ...
    pub fn push<I: AnchorSerialize>(&mut self, data: &mut [u8], item: I) -> Result<()> {
        ...
        require_lt!(self.len(), capacity, SolisticError::ListOverflow);
