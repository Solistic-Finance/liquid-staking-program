...
pub struct State {
    ...
    pub ssol_mint_authority_bump_seed: u8,
    ...
    pub ssol_supply: u64, // Virtual value (may be < because of token burn). Use Update* to align
...
impl State {
    ...
    pub const SSOL_MINT_AUTHORITY_SEED: &'static [u8] = b"st_mint";
    ...
    pub fn find_ssol_mint_authority(state: &Pubkey) -> (Pubkey, u8) {
        Pubkey::find_program_address(
            &[&state.to_bytes()[..32], State::SSOL_MINT_AUTHORITY_SEED],
            &ID,
        )
    }
    ...
    pub fn get_treasury_ssol_balance<'info>(
        &self,
        treasury_ssol_account: &AccountInfo<'info>,
    ) -> Option<u64> {
        if treasury_ssol_account.owner != &spl_token::ID {
            msg!(
                "treasury_ssol_account {} is not a token account",
                treasury_ssol_account.key
            );
            return None; // Not an error. Admins may decide to reject fee transfers to themselves
        }

        match spl_token::state::Account::unpack(treasury_ssol_account.data.borrow().as_ref()) {
            Ok(token_account) => {
                if token_account.mint == self.ssol_mint {
                    Some(token_account.amount)
                } else {
                    msg!(
                        "treasury_ssol_account {} has wrong mint {}. Expected {}",
                        treasury_ssol_account.key,
                        token_account.mint,
                        self.ssol_mint
                    );
            ...
            Err(e) => {
                msg!(
                    "treasury_ssol_account {} can not be parsed as token account ({})",
                    treasury_ssol_account.key,
                    e
                );
                None // Not an error. Admins may decide to reject fee transfers to themselves
    ...
    /// calculate the amount of ssol tokens corresponding to certain lamport amount
    pub fn calc_ssol_from_lamports(&self, stake_lamports: u64) -> Result<u64> {
        shares_from_value(
            stake_lamports,
    ...
    /// calculate lamports value from some ssol_amount
    /// result_lamports = ssol_amount * ssol_price
    pub fn ssol_to_sol(&self, ssol_amount: u64) -> Result<u64> {
        value_from_shares(
            ssol_amount,
            self.total_virtual_staked_lamports(),
            self.ssol_supply,
    ...
    pub fn on_ssol_mint(&mut self, amount: u64) {
        self.ssol_supply += amount
    }

    pub fn on_ssol_burn(&mut self, amount: u64) {
        self.ssol_supply -= amount
    }
    ...
    pub fn on_stake_moved(&mut self, amount: u64, clock: &Clock) -> Result<()> {
        ...
        require_lte!(
            ...
            SolisticError::MovingStakeIsCapped
        );
        Ok(())
    }
}
