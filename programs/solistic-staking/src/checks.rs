...
pub fn check_stake_amount_and_validator(
    ...
    let currently_staked = if let Some(delegation) = stake_state.delegation() {
        require_keys_eq!(
            ...
            *validator_vote_pubkey,
            SolisticError::WrongValidatorAccountOrIndex
        );
        delegation.stake
    } else {
        return err!(SolisticError::StakeNotDelegated);
    };
    ...
    if currently_staked != expected_stake_amount {
        ...
        return err!(SolisticError::StakeAccountNotUpdatedYet);
    }
    Ok(())
}
...
pub fn check_token_source_account<'info>(
    ...
    if source_account.delegate.contains(authority) {
        ...
        // delegated_amount & delegate must be set on the user's ssol account before calling OrderUnstake
        require_lte!(
            token_amount,
        ...
        return err!(SolisticError::WrongTokenOwnerOrDelegate)
