...
pub struct AddLiquidity<'info> {
    ...
    // liq_pool_ssol_leg to be able to compute current ssol value in liq_pool
    #[account(address = state.liq_pool.ssol_leg)]
...
impl<'info> AddLiquidity<'info> {
    ...
    pub fn process(&mut self, lamports: u64) -> Result<()> {
        ...
        require_lte!(
            ...
            SolisticError::UnregisteredLPMinted
        );
        ...
        // liq_pool_total_value = liq_pool_sol_account_pda.lamports() + value_from_ssol_tokens(liq_pool_ssol_account.token.balance)
        ...
        let ssol_leg_value = self.state.ssol_to_sol(self.liq_pool_ssol_leg.amount)?;
        let total_liq_pool_value = sol_leg_available_balance + ssol_leg_value;
        msg!(
            "liq_pool SOL:{}, liq_pool sSOL value:{} liq_pool_value:{}",
        ...
        // mint liq-pool shares (sSOL-SOL-LP tokens) for the user
