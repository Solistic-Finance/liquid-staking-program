...
pub struct ChangeAuthorityData {
    ...
    pub treasury_ssol_account: Option<Pubkey>,
...
impl<'info> ChangeAuthority<'info> {
    pub fn process(&mut self, data: ChangeAuthorityData) -> Result<()> {
        ...
        let treasury_ssol_account_change =
            if let Some(treasury_ssol_account) = data.treasury_ssol_account {
                let old = self.state.treasury_ssol_account;
                self.state.treasury_ssol_account = treasury_ssol_account;
                Some(PubkeyValueChange {
                    old,
                    new: treasury_ssol_account,
                })
            } else {
                None
            };
